...
13. الشرائح المتبقية (Contracts, Chat, Meetings, Clients, ...) لسه على النمط القديم — تتنقل شريحة-بشريحة بنفس المنهجية (اختبار توصيفي → hooks → نقل مكوّنات → تحقق) في جلسات لاحقة. Chat بالذات فيها تعقيد إضافي (polling كل 60 ثانية + WebSocket subscription)، فمرشحة لتُنقل بعناية أكبر من الشرائح البسيطة اللي اتنقلت لحد دلوقتي.
