...
**الجولة 3 اتقفلت بالكامل.** كل الشرائح السبعة (Payments, Files, Approvals, Meetings, Contracts, Clients, Chat) بقت على TanStack Query، والـ129 استدعاء `api.*` المتفرقة الأصلية بقت خلف hooks موحّدة قابلة لإعادة الاستخدام مع cache مشترك بين المكوّنات اللي بتضرب نفس الـendpoint. الـpolling اليدوي (`setInterval`) اتلغى في كل مكان واتحول لـ`refetchInterval` بتاع TanStack Query، والفشل الصامت في القراءة اتحل عبر `isLoading`/`isError` الموحّدة.
