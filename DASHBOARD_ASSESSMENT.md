...
16. الشريحة المتبقية الوحيدة (**Chat**) لسه على النمط القديم — تتنقل بنفس المنهجية (اختبار توصيفي → hooks → نقل مكوّنات → تحقق) في جلسة لاحقة. فيها تعقيد إضافي (polling كل 60 ثانية + WebSocket subscription عبر `subscribeToWorkspace`)، فمرشحة لتُنقل بعناية أكبر من الشرائح اللي اتنقلت لحد دلوقتي — وهي آخر شريحة في الجولة 3.
