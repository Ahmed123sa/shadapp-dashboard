  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-[var(--color-card)] rounded-xl border border-[var(--color-card-border)] p-6 space-y-6">
        ...
        {/* نوع العميل */}
        <div>
          <p className="text-[10px] tracking-wider font-medium text-[var(--color-text-muted)] uppercase mb-2">نوع العميل</p>
          <div className="grid grid-cols-2 gap-3">
            {(['business', 'individual'] as const).map(t => {
              const active = form.client_type === t;
              const isBiz = t === 'business';
              return (
                <button type="button" key={t} onClick={() => setForm(f => ({ ...f, client_type: t }))}
