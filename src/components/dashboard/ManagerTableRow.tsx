...
export default function ManagerTableRow({ manager, index, expanded, onToggle }: {
  ...
  // ملحوظة: كانت مكتوبة 'var(--gold-soft)' من غير بادئة --color- الصحيحة —
  // توكن غير معرّف إطلاقًا فكانت الخلفية بتتجاهل بصمت. صلّحتها لـ
  // --color-gold-soft الموجود فعليًا في globals.css.
  const badge = badgeStatus[pending] || { bg: 'var(--color-gold-soft)', color: 'var(--color-gold-text)' };
    ...
    >
      <td className="px-3.5 py-2.5">
        <div className="flex items-center gap-2">
          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[length:var(--fs-1)] font-bold flex-shrink-0 overflow-hidden ${expanded ? 'ring-2 ring-[var(--color-gold)]' : ''}`}
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[length:var(--fs-2)] font-bold truncate">{manager.name}</div>
            {manager.email && <div className="text-[length:var(--fs-1)] text-[var(--color-text-secondary)] truncate">{manager.email}</div>}
          </div>
      ...
      <td className="px-3.5 py-2.5 text-[length:var(--fs-1)] text-[var(--color-text-secondary)]">{manager.managed_clients_count}</td>
      <td className="px-3.5 py-2.5">
        <span className="px-2 py-0.5 rounded-full text-[length:var(--fs-1)] font-semibold" style={{ background: badge.bg, color: badge.color }}>
          {pending}
        </span>
      </td>
    </tr>
